...
import { EndorserCard as EndorserCardView } from '../components/EndorserCard';
import { ExpandedEndorserCard } from '../components/ExpandedEndorserCard';
import {
  buildEndorserCards,
  type EndorserCard,
} from '../components/endorserCardData';
import { DEMO } from '../../../demo/config';
import { recommendationsApi, referralsApi } from '../api';
import { referrerRecommendationsToEndorserCards } from '../mappers';
...
async function loadEndorserQueue(): Promise<EndorserCard[]> {
  if (DEMO.enabled) return buildEndorserCards('1');
  const recommendations = await recommendationsApi.getReferrers({ limit: 50 });
  return referrerRecommendationsToEndorserCards(recommendations);
}

function emptyTitleFor(state: QueueState): string {
  if (state === 'loading') return 'Finding matches';
  if (state === 'error') return 'Could not load matches';
  return "You're caught up";
}

function emptyBodyFor(state: QueueState): string {
  if (state === 'loading') return 'Building your referral match queue.';
  if (state === 'error') return 'Refresh to try loading your match queue again.';
  return (
...
function reportSeekerQueueError(error: unknown): void {
  if (__DEV__) {
    console.warn('Failed to load seeker discovery queue', error);
  }
}
...
export function SeekerSwipeDiscoverScreen(): React.ReactElement {
  const [queueKey, setQueueKey] = useState(0);
  const [allCards, setAllCards] = useState<EndorserCard[]>([]);
        ...
        .recordSeekerSwipe(
          {
            ...
            jobTitle: card.opportunityTitle ?? card.jobTitle,
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.deckFrame}>
          <SwipeDeck<EndorserCard>
            ...
            emptyTitle={emptyTitleFor(queueState)}
            emptyBody={emptyBodyFor(queueState)}
            renderCard={({
              item,
              isTop,
              stackIndex,
