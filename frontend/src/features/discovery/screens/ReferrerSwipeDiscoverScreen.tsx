...
import { SeekerCard as SeekerCardView } from '../components/SeekerCard';
import { ExpandedSeekerCard } from '../components/ExpandedSeekerCard';
import {
  MatchRevealModal,
  type MatchRevealData,
} from '../components/MatchRevealModal';
import {
  buildSeekerCards,
  type SeekerCard,
} from '../components/seekerCardData';
import { Phrase } from '../../../utils/haptics';
import { prefetchImages } from '../../../utils/prefetchImages';
import { recommendationsApi, referralsApi } from '../api';
import { seekerRecommendationsToSeekerCards } from '../mappers';
import { DEMO, referrerById } from '../../../demo/config';
import { useAuth } from '../../../hooks/useAuth';
import { shouldShowMatchReveal } from '../logic/matchReveal';
 ...
 */
export function ReferrerSwipeDiscoverScreen(): React.ReactElement {
  const { user } = useAuth();
  const [queueKey, setQueueKey] = useState(0);
  const [allCards, setAllCards] = useState<SeekerCard[]>([]);
  ...
  useEffect(() => {
    ...
    loadSeekerQueue()
      ...
      .catch((error: unknown) => {
        reportReferrerQueueError(error);
        if (cancelled) return;
        setAllCards([]);
        setQueueState('error');
      });
      ...
      if (direction === 'request') {
        void referralsApi
          .recordEndorserSwipe({
            ...
            skills: card.skills,
            targetRole: card.opportunityTitle ?? card.targetRole,
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.deckFrame}>
          <SwipeDeck<SeekerCard>
            ...
            emptyTitle={emptyTitleFor(queueState, expFilter)}
            emptyBody={emptyBodyFor(queueState, expFilter)}
            renderCard={({
              item,
              isTop,
              stackIndex,
...
async function loadSeekerQueue(): Promise<SeekerCard[]> {
  if (DEMO.enabled) return buildSeekerCards('2');
  const recommendations = await recommendationsApi.getSeekers({ limit: 50 });
  return seekerRecommendationsToSeekerCards(recommendations);
}

function emptyTitleFor(state: QueueState, filter: ExpFilter): string {
  if (state === 'loading') return 'Finding candidates';
  if (state === 'error') return 'Could not load candidates';
  if (filter === 'all') return 'Inbox empty';
  return `No ${EXP_LABEL[filter].toLowerCase()} candidates`;
}

function emptyBodyFor(state: QueueState, filter: ExpFilter): string {
  if (state === 'loading') return 'Building your candidate match queue.';
  if (state === 'error') return 'Refresh to try loading your queue again.';
  if (filter === 'all') return 'No more candidates in your queue.';
  return 'Try a different experience level, or refresh to see the full queue.';
}

function reportReferrerQueueError(error: unknown): void {
  if (__DEV__) {
    console.warn('Failed to load referrer discovery queue', error);
  }
}
